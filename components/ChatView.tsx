...
import { createChatSession, sendMessageToAI, checkSensitiveKeywords, ChatAttachment, fileToBase64 } from '../services/geminiService';
...
const ChatView: React.FC<ChatViewProps> = ({ onBack, onPaymentGate }) => {
  ...
  const handleSend = async (text: string, isSystemStart = false) => {
    ...
    if (!apiError && !isSystemStart) {
        // [NEW] Include indicator for attachment in UI
        let displayVerify = text;
        if (attachment) {
            displayVerify = `${attachment.mimeType.startsWith('video') ? '[视频附件] ' : '[图片附件] '}${text}`;
        }
        
        const newMsg: ChatMessage = { id: Date.now().toString(), role: 'user', text: displayVerify, timestamp: Date.now() };
    ...
    try {
      const rawResponse = await sendMessageToAI(chatSessionRef.current, msgToSend, currentMsgs, attachmentToSend);
  ...
  return (
    <Layout headerTitle="" showBack={false} hideHeader={true} disableScroll={true}>
      ...
      <div className="flex flex-col h-full bg-[#F7F8FA] w-full relative pb-[calc(60px+env(safe-area-inset-bottom))]">
            ...
            {messages.map((msg, index) => (
                <div key={msg.id} className="flex flex-col gap-3 animate-slide-up">
                    ...
                    <div className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start items-start gap-3'}`}>
                        ...
                        <div className={`max-w-[85%] p-4 rounded-2xl text-[15px] leading-relaxed shadow-sm whitespace-pre-wrap break-words ${
                            msg.role === 'user' 
        ...
        {/* [NEW] Attachment Preview Bubble */}
        {previewUrl && !isTakeoverMode && (
            <div className="flex-none px-4 pt-2 pb-0 z-20 animate-slide-up">
                <div className="relative inline-block">
                    {attachment?.mimeType.startsWith('video') ? (
                        <video src={previewUrl} className="h-20 w-auto rounded-xl border border-slate-200 object-cover" />
                    ) : (
                        <img src={previewUrl} alt="attachment" className="h-20 w-auto rounded-xl border border-slate-200 object-cover" />
                    )}
                    <button 
                        onClick={clearAttachment}
                        className="absolute -top-2 -right-2 bg-slate-800 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold shadow-sm"
                    >
                        ✕
                    </button>
                </div>
            ...
            <div className={`flex-none border-t p-3 z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.03)] transition-colors duration-300 ${isTakeoverMode ? 'bg-slate-100 border-slate-200' : 'bg-white border-slate-100'}`}>
                <div className="flex items-center gap-2">
                    ...
                    <input 
                        ...
                        disabled={isTakeoverMode}
                    />
                    <button 
                        onClick={() => fileInputRef.current?.click()}
                        disabled={isTakeoverMode}
                        className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-all active:scale-95 ${isTakeoverMode ? 'text-slate-300' : 'text-slate-500 hover:bg-slate-50'}`}
                    >
                    ...
                    <input
                        ...
                        placeholder={isTakeoverMode ? "⚠️ 医助正在接入... 请保持电话畅通" : attachment ? "添加描述 (可选)..." : "手动输入症状细节..."}
                        disabled={isTakeoverMode} 
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter' && !isTakeoverMode && (input.trim() || attachment)) {
                                handleSend(input);
                            }
                        }}
                    />
